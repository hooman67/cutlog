...
const MATERIAL_CONFIGS = {
  'Stainless Steel 304': {
    ...
    gasTypes: ['O2', 'N2', 'air'], // O2 for speed, N2 for quality
...
function generateDateInLastNDays(days) {
  const now = new Date();
...
function generateParameterSet(material, thickness, config) {
...
function generateAllParameterSets() {
  const allSets = [];
...
function generateSQLInserts(parameterSets) {
  const inserts = [];
...
function generateJSON(parameterSets) {
  return JSON.stringify(parameterSets, null, 2);
}

function printExamples(parameterSets, count = 5) {
  console.log(`\n${'='.repeat(80)}`);
...
function main() {
  ...
  const outputDir = args.includes('--output') ? args[args.indexOf('--output') + 1] : './data';
  ...
  console.log(`\nUsage:`);
  console.log(`  npx node scripts/generate-baseline-data.js            # Generate both JSON + SQL`);
  console.log(`  npx node scripts/generate-baseline-data.js --json     # JSON only`);
  console.log(`  npx node scripts/generate-baseline-data.js --sql      # SQL only`);
  console.log(`  npx node scripts/generate-baseline-data.js --dry-run  # Preview only`);
  console.log(`  npx node scripts/generate-baseline-data.js --output ./custom/path`);
}
